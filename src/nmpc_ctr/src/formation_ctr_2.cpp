...
{
    //初始化ros节点
    ros::init(argc, argv, "formation_ctr_2");
    ...
    ros::Subscriber sub2 = n.subscribe("/robot2/odom",100,&CurrentStatesListener::callback,&mlistener2);

    //创建控制量发布对象
    ros::Publisher controls_pub =n.advertise<geometry_msgs::Twist>("/robot2/cmd_vel",100);

    //创建预测轨迹发布对象
    ros::Publisher path_pub =n.advertise<nav_msgs::Path>("/robot2/predict_trajectory",100);

    //创建rviz目标点可视化发布对象
    ros::Publisher marker_pub =n.advertise<visualization_msgs::Marker>("/robot2/goal_marker",100);
    ...
    while(ros::ok())
    {
        ...
        //获取跟随者小车robot2状态
        Eigen::Matrix<float,3,1> robot2_states = mlistener2.m_current_states;
        float rb2_x =  robot2_states[0];
        float rb2_y =  robot2_states[1];
        float rb2_psi =  robot2_states[2];
        ...
        xd_from_map = rb1_x-0.5;
        yd_from_map = rb1_y-0.5;
